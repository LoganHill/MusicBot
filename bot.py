...
class MusicBot(discord.Client):
    ...
    async def select_lottery_winner(self, server, channel):
    	players = list(server.members)
    	players = [player for player in players if player.status != discord.Status.offline]
    	numPlayers = len(players)

    	winner = players[randint(0, numPlayers - 1)]

    	# You can change your "win" message here
    	await self.safe_send_message(channel, "[Lottery] " + winner.name + " won the lottery out of " + str(numPlayers) + " players!")

    async def do_lottery(self, server, channel, interval):

    	while self.lottery_running:
    		await asyncio.sleep(interval) # sleep for interval and then run again

    		# this is because we are probably sleeping this coroutine inside the loop while we stop the lottery
    		if not self.lottery_running:
    			break

    		# select winner
    		await self.select_lottery_winner(server, channel)

    @owner_only
    async def cmd_startlottery(self, message, channel, server, author, interval=10):
        try:
            int(interval)
        except:
            return Response("enter a number.  NUMBER.  That means digits.  `15`.  Etc.", reply=True, delete_after=8)

        await self.safe_send_message(channel, "Starting user lottery with " + interval + " second interval")

        self.lottery_running = True # start things
        await self.do_lottery(server, channel, int(interval))    

    @owner_only
    async def cmd_stoplottery(self, message, channel, server, author):
    	self.lottery_running = False
